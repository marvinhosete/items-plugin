...
import static me.tuskdev.items.inventory.PaginatedView.*;
import static me.tuskdev.items.inventory.View.INVENTORY_ROW_SIZE;
...
public class VirtualView {
	...
	/**
	 * Registers a {@link ViewItem} in the specified row and first column.
	 *
	 * @param slot the item slot.
	 */
	public final ViewItem atSlot(int slot) {
		inventoryModificationTriggered();
		final int max = getLastSlot() + 1;
		if (slot > max)
			throw new IllegalArgumentException(
