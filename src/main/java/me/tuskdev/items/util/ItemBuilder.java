...
public class ItemBuilder {
    ...
    /**
     * Makes a complete copy of an {@link ItemStack} with its latest changes.
     *
     * @param itemStack item stack to copy
     */
     ...
     * <p/>
     * <b>WARNING: This method WILL call {@link Bukkit#getItemFactory()#getItemMeta(Material)} to copy the
    ...
    public List<String> getLore() {
        return Collections.unmodifiableList(itemMeta.getLore());
    }
    ...
    /**
     * Adds a lore, by index, from this item.
     *
     * @param index lore index to add
     * @param lore  lore to add
     * @return this item builder instance, for chaining
     */
    public ItemBuilder addLore(int index, String lore) {
        try {
            ...
            if (loreList.size() <= index) loreList.add(lore);
            else loreList.set(index, lore);
            this.itemMeta.setLore(loreList);
        } catch (Exception e) {
            if (!this.failSilently) {
                e.printStackTrace();
    ...
    /**
     * Adds the {@link Enchantment#LURE} to this item, making it glow as enchanted,
    ...
    /**
     * Sets whether this item is to glow using {@link Enchantment#LURE}, making it
    ...
    public ItemBuilder glow(boolean glow) {
        if (glow) {
            return enchant(Enchantment.LURE, 4341, true);
        } else {
            return removeEnchant(Enchantment.LURE);
        }
    }
    ...
    public ItemBuilder unbreakable(boolean unbreakable) {
        try {
            this.itemStack.setDurability((short) (unbreakable ? 1 : 0));
        } catch (Exception e) {
            if (!this.failSilently) {
                e.printStackTrace();
            }
    ...
    public static ItemStack load(ConfigurationSection configurationSection) {
        ItemBuilder itemBuilder = (configurationSection.contains("url") ? new ItemBuilder(CustomHead.skull(configurationSection.getString("url"))) : new ItemBuilder(Material.getMaterial(configurationSection.getString("material", "stone").toUpperCase()), configurationSection.getInt("amount", 1), (byte) configurationSection.getInt("durability", 0)))
                .name(ChatColor.translateAlternateColorCodes('&', configurationSection.getString("name", "")))
                .lore(StringUtil.translateAlternateColorCodes('&', configurationSection.getStringList("lore")));

        if (configurationSection.contains("owner")) itemBuilder.skull(configurationSection.getString("owner"));
