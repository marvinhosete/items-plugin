...
public class WeaponsAttributesView extends View {

    private final Configuration itemsConfiguration;

    public WeaponsAttributesView(Configuration itemsConfiguration) {
        super(3, "Atributos de Armaduras");
    ...
    protected void onRender(ViewContext context) {
        ...
        ItemAttribute[] weaponsAttributes = ItemAttribute.getWeaponsAttributes();
        for (int i = 0; i < weaponsAttributes.length; i++) {
            ItemAttribute attribute = weaponsAttributes[i];
            int value = customItem.getAttribute(attribute);

            context.slot(i*2+10, ItemBuilder.builder(Material.INK_SACK).durability(value == 0 ? 8 : 10).name("§a" + attribute.getDescription()).lore("§7Valor atual: §f" + (value == 0 ? "Nenhum" : value), "", "§7Clique para alterar o valor.").build()).onClick(handler -> {
                handler.closeNow();
